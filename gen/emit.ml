...
let make_record_decl ~selected ~alias_tbl ~domain ~type_name ~attrs ~hoist_name props =
  let field prop =
    ...
    in
    let line = spf "  %s : %s%s%s;" (sanitize_lower orig) ocaml_type wire_attrs (Attributes.render_field prop) in
    line, hoisted
  in
...
let collect_named_type_decls ~selected ~alias_tbl ~domain type_def =
  ...
  let attrs = Attributes.render_item type_def in
      ...
      | aliases ->
        ...
        List.iter
          (fun (type_def, prim) ->
            ...
            Buffer.add_string buf (render_sealed_module ~module_name ~prim ~attrs:(Attributes.render_item type_def));
            Buffer.add_string buf "\n")
          aliases;
    ...
    (fun (type_def, _prim) ->
      ...
      Buffer.add_string buf
        (spf "module %s = %s%s\n" module_name
           ...
           (Attributes.render_item type_def)))
      ...
      | `Command command ->
        Buffer.add_string buf
          (emit_item_module ~selected ~alias_tbl ~domain:domain.name ~hoist_name ~module_name
             ...
             ~attrs:(Attributes.render_item command) ~params:(get_list "parameters" command)
             ~returns:(`Returns (get_list "returns" command)))
      | `Event event ->
        Buffer.add_string buf
          (emit_item_module ~selected ~alias_tbl ~domain:domain.name ~hoist_name ~module_name
             ~wire_name:(spf "%s.%s" domain.name (get_string "name" event))
             ~attrs:(Attributes.render_item event) ~params:(get_list "parameters" event) ~returns:`Event))
...
let emit_index ~revision domains =
  ...
  List.iter
    (fun (domain : domain) ->
      Buffer.add_string buf
        (spf "module %s = %s%s\nmodule %s_types = %s%s\n" (module_of_domain domain.name)
           (domain_module_of_domain domain.name) (Attributes.render_domain domain) (module_of_domain domain.name)
           (types_module_of_domain domain.name) (Attributes.render_domain domain)))
    domains;
  Buffer.contents buf
