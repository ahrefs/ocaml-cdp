...
let render_flags ~marker ~separator { deprecated; experimental } =
  let deprecated_attr =
    match deprecated with
    | false -> ""
    | true -> spf "%s[%socaml.deprecated \"deprecated in CDP\"]" separator marker
  in
  let experimental_attr =
    match experimental with
    | false -> ""
    | true -> spf "%s[%salert experimental \"experimental in CDP, may change with Chrome\"]" separator marker
  in
